...
struct lreq_parser : base_parser<lreq_parser<pos_in_variant>> {
	constexpr auto parse(auto&& ctx,auto,auto&) const requires (
		   ascip_details::is_in_concept_check(decltype(auto(ctx)){}) ){ return 0; }
	constexpr auto parse_next_variant(auto&& ctx, auto src, auto& result) const {
		auto* next_variant = search_in_ctx<variant_stack_tag>(ctx);
		return next_variant->template parse_from<1>(ctx, src, result);
	}
	...
	constexpr auto count_reqursion_expressions(auto start_pos, auto&& ctx, auto src, auto& result) const {
			...
			while(0 <= cur) {
				++expr_count;
				acc += cur;
				cur = parse_current_parser<1>(acc, ctx, src, result);
			}
...
template<auto pos_in_variant> constexpr static auto lreq = lreq_parser<pos_in_variant>{};
